# -*- coding: utf-8 -*-
"""
Created on Fri Aug 12 16:48:09 2022
...
def pitcher_avg_heatmap(pid, split='all', perspective='pitcher', ptype='all'):
    ...
    if split == 'all':
        temp = pd.read_sql_query(
            f'''SELECT * FROM savant_convert WHERE PitcherId = {pid}''', conn)
        conn.close()

    elif split == 'left':
        temp = pd.read_sql_query(
            f'''SELECT * FROM savant_convert WHERE PitcherId = {pid} AND BatterSide = 'Left' ''', conn)
        conn.close()

    elif split == 'right':
        temp = pd.read_sql_query(
            f'''SELECT * FROM savant_convert WHERE PitcherId = {pid} AND BatterSide = 'Right' ''', conn)
        conn.close()
...
pitcher_avg_heatmap(pid=pid, split=split, perspective='catcher', ptype=ptype)
